...
// The client callback is registered through a variadic message dispatcher.
// The first parameter is the Fade message; the remaining slots are part of
// the dispatcher context and must stay present when the trampoline is called.
using NativeFade_t = __int64 (__fastcall *)(
    void * message,
    void * arg2,
    void * arg3,
    void * arg4,
    ...
    void * arg8,
    void * arg9);
// IDA confirms this is CViewEffects::Get(int splitScreenSlot). The remaining
// bytes in the native prototype are only vararg call-site noise; the function
// resolves the active split-screen instance from the first argument.
using CViewEffects_Get_t = void * (__fastcall *)(int splitScreenSlot);
using CViewEffects_AddFade_t = uint32_t * (__fastcall *)(
    void * effects,
...
NativeFade_t g_NativeFade = nullptr;
bool g_NativeFadeHooked = false;
std::atomic<bool> g_NativeFadeDisabled { false };
CViewEffects_Get_t g_CViewEffectsGet = nullptr;
...
std::atomic<bool> g_DeathFadeObserverKnown { false };
std::atomic<uint8_t> g_DeathFadeObserverMode { 0 };
    ...
    uint32_t & rgba)
{
    ...
    __try {
        memcpy(&duration, (unsigned char *)message + 0x48, sizeof(duration));
        memcpy(&hold, (unsigned char *)message + 0x4C, sizeof(hold));
        memcpy(&flags, (unsigned char *)message + 0x50, sizeof(flags));
        memcpy(&rgba, (unsigned char *)message + 0x54, sizeof(rgba));
        return true;
...
static bool NativeFade_IsBlack(uint32_t rgba, uint16_t flags) {
    ...
    return c[0] < 8 && c[1] < 8 && c[2] < 8
        && (0 != (flags & (kFadeOut | kFadeStayOut)));
}

static void NativeFade_ObserveGameMessage(
    uint16_t duration,
    uint16_t hold,
    uint16_t flags,
    uint32_t rgba)
{
    // Only learn from the actual game. Synthetic calls set
    // g_NativeFadeApplySource and must never feed the template back into
    // themselves.
    ...
    if (NativeFade_IsRed(rgba) && g_HurtEventAwaitingFade.exchange(false, std::memory_order_acq_rel)) {
        ...
        const float deathCurtime = g_LastDeathEventCurtime.load(std::memory_order_acquire);
        const float now = g_MirvTime.curtime_get();
        if (0.0f <= deathCurtime && now >= deathCurtime) {
            const float delay = now - deathCurtime;
            if (delay <= 2.0f) {
                g_ObservedDeathBlackDelay.store(delay, std::memory_order_release);
            }
        }

        int deathTick = g_LastDeathEventTick.load(std::memory_order_acquire);
        int fadeTick = -1;
        if (0 <= deathTick && g_MirvTime.GetCurrentDemoTick(fadeTick)
            && fadeTick >= deathTick && fadeTick - deathTick <= 128) {
            g_ObservedDeathBlackDelayTicks.store(fadeTick - deathTick, std::memory_order_release);
        }
    }
}
...
static __int64 __fastcall NativeFade_Intercept(
    ...
    void * arg9)
{
    NativeFade_t original = g_NativeFade;
    ...
    uint16_t flags = 0;
    uint32_t rgba = 0;
    const bool readable = NativeFade_ReadMessage(message, duration, hold, flags, rgba);
    if(readable) NativeFade_ObserveGameMessage(duration, hold, flags, rgba);

    return original(message, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9);
}

bool NativeFade_Apply(
    const char * source,
    uint16_t duration,
    uint16_t hold,
    uint16_t flags,
    uint32_t rgba,
    int requestedEffectId = -1,
    uint32_t * appliedEffectId = nullptr) {
    if(nullptr == g_CViewEffectsGet || g_NativeFadeDisabled.load(std::memory_order_acquire)) {
        return false;
    }
...
#pragma pack(pop)
    ...
    __try {
        void * effects = g_CViewEffectsGet(-1);
        if (nullptr == effects) {
            g_NativeFadeApplySource = previousFadeApplySource;
            return false;
        }

        void ** vtable = *reinterpret_cast<void ***>(effects);
        if (nullptr == vtable || nullptr == vtable[6]) {
            g_NativeFadeApplySource = previousFadeApplySource;
            return false;
        }
        auto addFade = reinterpret_cast<CViewEffects_AddFade_t>(vtable[6]);
        uint32_t effectId = 0;
        uint32_t * result = addFade(
            effects,
            &effectId,
            reinterpret_cast<uint16_t *>(&payload),
            requestedEffectId);
        if (nullptr != appliedEffectId) *appliedEffectId = effectId;
    } __except(EXCEPTION_EXECUTE_HANDLER) {
        g_NativeFadeApplySource = previousFadeApplySource;
        g_NativeFadeDisabled.store(true, std::memory_order_release);
        return false;
    }
    g_NativeFadeApplySource = previousFadeApplySource;
    return true;
...
static void RenderSystemDX11_DeathFade_InitializeViewEffects(void * clientDll) {
    if (nullptr == clientDll) return;
    if (nullptr == g_CViewEffectsGet) {
        const char * getterPattern =
            // sub_180BA7CD0 in the current client: CViewEffects::Get(-1)
            // lazily initializes the global array, resolves the local slot,
            // then returns the slot object at +0xA8 * index.
            "53 48 83 EC 20 65 48 8B 04 25 58 00 00 00 "
            "8B D9 8B 0D ?? ?? ?? ?? BA 68 00 00 00 48 8B 04 C8 "
            "8B 04 02";
        size_t address = getAddress((HMODULE)clientDll, getterPattern);
        if (0 != address) g_CViewEffectsGet = reinterpret_cast<CViewEffects_Get_t>(address);
    }
}

static bool RenderSystemDX11_DeathFade_ClearNative(const char * source) {
    if (nullptr == g_CViewEffectsGet) return false;

    __try {
        void * effects = g_CViewEffectsGet(-1);
        if (nullptr == effects) return false;
        void ** vtable = *reinterpret_cast<void ***>(effects);
        if (nullptr == vtable || nullptr == vtable[10]) return false;
        auto clearFade = reinterpret_cast<CViewEffects_ClearFade_t>(vtable[10]);
        clearFade(effects);
        return true;
    } __except(EXCEPTION_EXECUTE_HANDLER) {
        return false;
...
void RenderSystemDX11_DeathFade_Initialize(void * clientDll) {
    ...
    RenderSystemDX11_DeathFade_InitializeViewEffects(clientDll);

    if(nullptr != g_NativeFade) {
        return;
    }

    // Unique in the current client.dll IDA database (cs2_client_20260804):
    // sub_180BB91A0, the registered CUserMessageFade_t callback.
    const char * pattern =
        "48 83 EC 38 0F B7 41 48 66 89 44 24 ?? "
        "0F B7 41 4C 66 89 44 24 ?? 0F B7 41 50 "
        "66 89 44 24 ?? 8B 41 54 8B 0D ?? ?? ?? ??";
    size_t address = getAddress((HMODULE)clientDll, pattern);
    if(0 == address) {
        return;
    }

    g_NativeFade = reinterpret_cast<NativeFade_t>(address);

    DetourTransactionBegin();
    DetourUpdateThread(GetCurrentThread());
    LONG attachResult = DetourAttach(&(PVOID&)g_NativeFade, NativeFade_Intercept);
    LONG commitResult = DetourTransactionCommit();
    if(NO_ERROR != commitResult) {
        g_NativeFade = reinterpret_cast<NativeFade_t>(address);
    } else {
        g_NativeFadeHooked = true;
    }
...
void RenderSystemDX11_DeathFade_Death() {
    ...
    const float observedDelay = g_ObservedDeathBlackDelay.load(std::memory_order_acquire);
    int observedDelayTicks = g_ObservedDeathBlackDelayTicks.load(std::memory_order_acquire);

    float delay = observedDelay;
    if (delay < 0.0f) delay = g_MirvTime.interval_per_tick_get();
    if (delay < 0.0f) delay = 0.0f;

    if (hasDemoTick) {
        if (observedDelayTicks < 0) {
            const float interval = g_MirvTime.interval_per_tick_get();
            observedDelayTicks = interval > 0.0f
                ? static_cast<int>((delay / interval) + 0.999f)
                : 1;
        }
        // A native message observed in the same simulation tick must still be
        // applied at the next engine boundary, not in the event callback.
        if (observedDelayTicks < 0) observedDelayTicks = 0;
        g_DeathFadeDueTick.store(tick + observedDelayTicks, std::memory_order_release);
    } else {
        g_DeathFadeDueTick.store(-1, std::memory_order_release);
    }
    g_DeathFadeDueCurtime.store(now + delay, std::memory_order_release);
    g_DeathBlackFadeEffectId.store(-1, std::memory_order_release);
    g_DeathFadeScheduledEpoch.store(epoch, std::memory_order_release);
}

void RenderSystemDX11_DeathFade_ObserveDeathEvent() {
    const float now = g_MirvTime.curtime_get();
    g_LastDeathEventCurtime.store(now, std::memory_order_release);
    g_ObservedDeathBlackDelay.store(-1.0f, std::memory_order_release);
    g_ObservedDeathBlackDelayTicks.store(-1, std::memory_order_release);
    g_DeathEventAwaitingBlack.store(true, std::memory_order_release);
    int tick = -1;
    if(g_MirvTime.GetCurrentDemoTick(tick)) {
        g_LastDeathEventTick.store(tick, std::memory_order_release);
    } else {
        g_LastDeathEventTick.store(-1, std::memory_order_release);
    }
}

void RenderSystemDX11_DeathFade_ResetObserverState() {
    g_DeathFadeObserverKnown.store(false, std::memory_order_release);
    g_DeathFadeObserverMode.store(0, std::memory_order_release);
    g_DeathFadeObserverTarget.store(0xFFFFFFFFu, std::memory_order_release);
    g_DeathFadeLastDemoTick = -1;
}

static void RenderSystemDX11_DeathFade_ClearOwned(const char * source, bool force) {
    ...
    // Game events can arrive while the renderer is traversing CViewEffects.
    // Defer the actual native list purge to the engine/render thread instead
    // of mutating the fade vector from the event callback thread.
    const bool alreadyPending = g_DeathFadeClearPending.exchange(
        true,
...
void RenderSystemDX11_DeathFade_Reset() {
    g_DeathFadeDueCurtime.store(-1.0f, std::memory_order_release);
    g_DeathFadeDueTick.store(-1, std::memory_order_release);
    // Reset is also used by spec_target_updated and shutdown paths. Do not
    // gate the purge on the current mirv_pov flag: the flag may already have
    // been cleared while our owned native fade is still active.
    RenderSystemDX11_DeathFade_ClearOwned("reset", false);
}

void RenderSystemDX11_DeathFade_ClearForObserverChange() {
    // A spectator target change is a native lifecycle boundary. Purge even
    // when our bookkeeping missed the fade-creation edge, otherwise a stale
    // black Fade entry can remain on the next watched player.
    RenderSystemDX11_DeathFade_ClearOwned("observer-change-event", true);
}
...
void RenderSystemDX11_DeathFade_UpdateObserverState() {
    ...
    int demoTick = -1;
    if(g_MirvTime.GetCurrentDemoTick(demoTick)) {
        if(g_DeathFadeLastDemoTick >= 0) {
            const int delta = demoTick - g_DeathFadeLastDemoTick;
            if(delta < 0 || delta > 2) {
                RenderSystemDX11_DeathFade_ClearOwned("demo-seek", true);
            }
        }
        g_DeathFadeLastDemoTick = demoTick;
    } else {
        g_DeathFadeLastDemoTick = -1;
    }
    ...
    bool available = MirvPov_GetObserverState(observerMode, observerTarget);
    if(!available) {
        if(g_DeathFadeObserverKnown.load(std::memory_order_acquire) || g_DeathFadeActive.load(std::memory_order_acquire)) {
            RenderSystemDX11_DeathFade_ClearOwned("observer-invalid", true);
            RenderSystemDX11_DeathFade_ResetObserverState();
        }
        return;
    }

    if(!g_DeathFadeObserverKnown.load(std::memory_order_acquire)) {
        g_DeathFadeObserverKnown.store(true, std::memory_order_release);
    ...
    const uint8_t previousMode = g_DeathFadeObserverMode.load(std::memory_order_acquire);
    const uint32_t previousTarget = g_DeathFadeObserverTarget.load(std::memory_order_acquire);
    if(previousMode != observerMode || previousTarget != observerTarget) {
        g_DeathFadeObserverMode.store(observerMode, std::memory_order_release);
        g_DeathFadeObserverTarget.store(observerTarget, std::memory_order_release);
...
void RenderSystemDX11_DeathFade_ProcessPending() {
    if (g_DeathFadeClearPending.load(std::memory_order_acquire)) {
        // CUserMessageFade_t with duration=0 used to leave an empty Fade entry
        // in CViewEffects. Use the native ClearFade virtual directly so the
        // entire fade list is released before the next spectator target is
        // rendered. This runs on the engine/render thread.
        if (!RenderSystemDX11_DeathFade_ClearNative("engine-thread-clear")) {
            // Keep the request pending. Sending a zero-duration synthetic Fade
            // leaves an empty native entry and does not reliably clear it.
            return;
        }
        g_DeathFadeClearPending.store(false, std::memory_order_release);
        g_DeathFadeActive.store(false, std::memory_order_release);
        // A successful purge is a frame boundary. Do not re-add a death Fade
        // in this same render pass while the observer target is changing.
        return;
    }
    ...
    if(dueCurtime < 0.0f) return;

    int currentTick = -1;
    const bool hasTick = g_MirvTime.GetCurrentDemoTick(currentTick);
    const int dueTick = g_DeathFadeDueTick.load(std::memory_order_acquire);
    // Demo playback has a discrete native tick clock. Using curtime OR tick
    // made the fade fire early as soon as curtime advanced, which removed the
    // normal red-to-black interval. Use exactly one clock for each mode.
    const bool due = hasTick && 0 <= dueTick
        ? currentTick >= dueTick
        : (!hasTick && g_MirvTime.curtime_get() >= dueCurtime);
    if(!due) return;

    float dueCurtimeExpected = dueCurtime;
    if(g_DeathFadeDueCurtime.compare_exchange_strong(
        dueCurtimeExpected,
        -1.0f,
        std::memory_order_acq_rel)) {
        // Observer changes and demo seeks invalidate the request while it is
        // waiting in the render queue. Re-check both epoch fields after the
        // consume CAS, immediately before touching CViewEffects.
        if (g_DeathFadeEpoch.load(std::memory_order_acquire) != scheduledEpoch
        ...
        if (NativeFade_Apply(
            ...
            death.flags,
            death.rgba,
            g_DeathBlackFadeEffectId.load(std::memory_order_acquire),
            &effectId)) {
            g_DeathBlackFadeEffectId.store(static_cast<int>(effectId), std::memory_order_release);
            g_DeathFadeActive.store(true, std::memory_order_release);

            // CViewEffects is composited after the player_death listener has
            // updated Panorama.  In POV playback the watched pawn is not the
            // real split-screen pawn, so the native observer transition can
            // leave the DeathPanel state prepared but not submitted in the
            // same frame as the black Fade.  Re-enter the game's own Show
            // routine after AddFade, on the engine/render thread, so the
            // banner is committed in the same ordering as the native death
            // presentation.  This is intentionally not done on observer
            // target changes: the banner belongs to the just-deceased POV.
            MirvPovDeathPanel_Reapply("engine-thread-after-death-black");
        } else if(g_DeathFadeEpoch.load(std::memory_order_acquire) == scheduledEpoch) {
            g_DeathFadeDueCurtime.store(dueCurtime, std::memory_order_release);
...
public:
    ...
    void ShutDown(ID3D11DeviceContext * pDeviceContext)
    {
        ...
        // Wait for all textures to finish / return.
        {
            std::unique_lock<std::mutex> lock(m_DoneTexturesMutex);
            m_DoneTexturesCv.wait(lock, [this]{return m_NumCpuTextures == m_CpuTexturesDone.size();});
        }
    ...
    void OnBeforeGpuPresent(ID3D11DeviceContext * pDeviceContext, ID3D11Texture2D * pTexture, float depthScale, float depthOfs) {
        if(nullptr == m_CurrentCpuTexture) {            
            ...
            if(pDevice) {
                ...
                else if(!m_CpuTexturesDone.empty()) {
                    m_CurrentCpuTexture = m_CpuTexturesDone.front();
                    m_CpuTexturesDone.pop();
                }
                if(nullptr == m_CurrentCpuTexture) {
...
private:
    ...
    std::condition_variable m_DoneTexturesCv;
    size_t m_NumCpuTextures = 0;
	...
	void ProcessingThreadFunc() {
		std::unique_lock<std::mutex> lock(m_ProcessingThreadMutex);
		while (!m_ShutDown || !m_CpuTexturesTodo.empty()) {
			if (!m_CpuTexturesTodo.empty()) {
                auto pTexture = m_CpuTexturesTodo.front();
...
static ID3D11Texture2D * AfxCapture_AcquirePresentTexture() {
    ...
    if(g_BeforeUiRT) {
        ...
        if(pRenderTargetViewResource) {
            if(FAILED(pRenderTargetViewResource->QueryInterface(__uuidof(ID3D11Texture2D),(void**)&pTexture))) {
                pTexture = nullptr;
            }
            pRenderTargetViewResource->Release();
        }
    ...
    // marker that fills g_BeforeUiRT often never runs. Fall back to swapchain backbuffer.
...
advancedfx::CGrowingBufferPoolThreadSafe g_ImageBufferPool;
...
void RenderSystemDX11_EngineThread_BeforeRender() {
    RenderSystemDX11_DeathFade_ProcessPending();
